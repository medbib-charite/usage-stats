...
```{r}
articles <- "+16 %"
valueBox(articles, icon = "fa-search-plus", color = "#36AFAEFF", href="#primo")
```
...
```{r}
comments <- "+28 %"
valueBox(comments, icon = "fa-user-check", color = "#36AFAEFF", href="#website")
```
...
p1 <- Website_OS %>%
  ...
  layout(title="Verwendung Betriebssysteme 2020")
p1
```
...
```{r}
Ausleihen <- read_excel("T:/Statistik/ALMA_ART/Ausleihen.xlsx", 
    sheet = "Ausleihen_R", col_types = c("text", 
...
```{r}
Ausleihen %>%
  mutate(Percent_Change = (Ausleihen_u_Verl/(lag(Ausleihen_u_Verl))-1))
```
